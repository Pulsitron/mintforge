...
import LaunchCollection from "./LaunchCollection";
import CollectionThumbnail from "./CollectionThumbnail";
import { basename, importMetadata, validateArtwork, type Metadata } from "../lib/collection-files";
import StudioNavigation from "./StudioNavigation";
import { destinationFromLocation, destinationHref, type StudioDestination, type StudioMode } from "../lib/studio-navigation";
import { chains } from "../lib/chains";
import { contract, deployment } from "../lib/web3";
import { parseTraitCsv, traitCsvTemplate } from "../lib/collection-import";
...
const headings = [
  ...
  { number: "06 / 06", title: <>Review the <em>protocol.</em></>, copy: "Check your files, price and permissions before deploying your collection." },
];

type FileKind = "image" | "video" | "audio" | "interactive" | "model" | "document" | "code" | "archive" | "file";
export type UploadedArtwork = { name: string; size: string; type: string; extension: string; kind: FileKind; url: string; previewText: string | null; file: File };
export type CollectionItem = { id: string; name: string; size: string; url: string; file: File; metadata?: Metadata; traitsEdited?: boolean; kind: FileKind; traits: { id: string; trait_type: string; value: string }[] };
...
export type Draft = {
  ...
  symbol: string;
  standard: "ERC-721" | "ERC-1155" | "ERC-404";
...
export default function MintForgeStudio({ initialDestination = "create" }: { initialDestination?: StudioDestination }) {
  const [mode, setMode] = useState<StudioMode>(initialDestination === "collection" ? "create" : initialDestination);
  ...
  const [draft, setDraft] = useState<Draft>({ ...initialDraft, itemMode: initialDestination === "collection" ? "collection" : "single" });
  ...
  useEffect(() => {
    const restoreLocation = () => {
      const destination = destinationFromLocation(location.pathname, location.hash);
      setMode(destination === "collection" ? "create" : destination);
      if (destination === "collection" || location.pathname === "/create") {
        setDraft((current) => ({ ...current, itemMode: destination === "collection" ? "collection" : "single" }));
        setActiveStep(0);
      }
    };
    restoreLocation();
  ...
  useEffect(() => {
    let active = true; setPlatformFee(null);
    deployment(selectedChain.id).then(async d => {
      if (!d) return;
      const f = await contract("MintForgeFactory", d.factory, selectedChain.id);
      const fee = Number(await f.feeBps()) / 100;
      if (active) setPlatformFee(fee);
    }).catch(() => {});
    return () => { active = false; };
  }, [selectedChain.id]);
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const restored = { ...initialDraft, ...JSON.parse(saved) } as Draft;
        const destination = destinationFromLocation(location.pathname, location.hash);
        if (destination === "collection") restored.itemMode = "collection";
        else if (location.pathname === "/create") restored.itemMode = "single";
        setDraft(restored);
      }
    } catch {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        setSavedAt("Draft stays in this tab");
      }
    }, 350);
  ...
  function acceptRevealPlaceholder(file?: File) {
    ...
    setRevealPlaceholder({ file, name: file.name, size: formatFileSize(file.size), type: file.type, extension: file.name.split(".").pop()?.toUpperCase() || "FILE", kind: "image", url: URL.createObjectURL(file), previewText: null });
  }

  async function acceptCsvTraits(file?: File) {
    if (!file) return;
    try {
      if (file.size > 4 * 1024 * 1024) throw new Error("Trait CSV must be smaller than 4 MiB.");
      const { byFilename, rowCount } = parseTraitCsv(await file.text());
      setCollectionItems((items) => items.map((item) => {
        const traits = byFilename.get(item.name.toLowerCase());
        return traits ? { ...item, traitsEdited: true, traits: traits.map((trait) => ({ ...trait, id: crypto.randomUUID() })) } : item;
      }));
      const matched = collectionItems.filter((item) => byFilename.has(item.name.toLowerCase())).length;
      setCsvSummary(`Matched ${matched} of ${rowCount} rows to uploaded files by filename.`);
    } catch (error) {
      setCsvSummary(error instanceof Error ? error.message : "Unable to read CSV.");
    }
  }

  function downloadCsvTemplate() {
    const blob = new Blob([traitCsvTemplate(collectionItems.map((item) => item.name))], { type: "text/csv" });
  ...
  async function acceptMetadataFiles(fileList: FileList | null) {
    ...
    setCsvSummary("Reading and validating metadata…");
    try {
      const metadata = await importMetadata(Array.from(fileList), collectionItems, n => {
        if (n % 250 === 0) setCsvSummary(`Validated ${n.toLocaleString()} JSON files…`);
      });
      setCollectionItems(items => items.map(item => {
        const data = metadata.get(basename(item.name));
        if (!data) return item;
        const attrs = Array.isArray(data.attributes) ? data.attributes : [];
        const traits = attrs.filter(a => a && typeof a === "object" && "value" in a)
          .map(a => ({id: crypto.randomUUID(), trait_type: String(a.trait_type ?? a.traitType ?? ""), value: String(a.value)}));
        return {...item, metadata: data, traits, traitsEdited: false};
      }));
      setCsvSummary(`Imported ${metadata.size.toLocaleString()} complete metadata JSON files. Names, descriptions, numeric traits and custom fields are preserved; media links are updated after upload.`);
    } catch (error) { setCsvSummary(error instanceof Error ? error.message : "Metadata import failed."); }
  }
  ...
  async function acceptCollectionFiles(fileList: FileList | null) {
    ...
    try {
      validateArtwork([...collectionItems.map(item => item.file), ...files]);
      const newItems: CollectionItem[] = files.map(file => ({
        id: crypto.randomUUID(), name: file.name, size: formatFileSize(file.size),
        url: "", file, kind: detectFileKind(file), traits: [],
      }));
      setCollectionItems(items => [...items, ...newItems].sort((a,b)=>a.name.localeCompare(b.name,undefined,{numeric:true})));
      setNotice("");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Artwork import failed."); }
  }
  ...
  function addItemTrait(itemId: string) {
    setCollectionItems((items) => items.map((item) => item.id === itemId ? { ...item, traitsEdited: true, traits: [...item.traits, { id: Math.random().toString(36).slice(2), trait_type: "", value: "" }] } : item));
  }
  function updateItemTrait(itemId: string, traitId: string, field: "trait_type" | "value", value: string) {
    setCollectionItems((items) => items.map((item) => item.id === itemId ? { ...item, traitsEdited: true, traits: item.traits.map((t) => t.id === traitId ? { ...t, [field]: value } : t) } : item));
  }
  function removeItemTrait(itemId: string, traitId: string) {
    setCollectionItems((items) => items.map((item) => item.id === itemId ? { ...item, traitsEdited: true, traits: item.traits.filter((t) => t.id !== traitId) } : item));
  }
  ...
  function acceptBanner(file?: File) {
    ...
    setBanner({ file, name: file.name, size: formatFileSize(file.size), type: file.type, extension: file.name.split(".").pop()?.toUpperCase() || "FILE", kind: "image", url: URL.createObjectURL(file), previewText: null });
  }
  ...
  async function acceptFile(file?: File) {
    ...
    if (file.size > 90 * 1024 * 1024) {
      setNotice("This interface currently accepts files up to 90 MB. Compress larger assets or use an external metadata URI.");
      return;
    }
    ...
    setArtwork({ file, name: file.name, size: formatFileSize(file.size), type: file.type || "application/octet-stream", extension, kind, url: URL.createObjectURL(file), previewText });
  ...
  function switchMode(destination: StudioDestination) {
    const nextMode = destination === "collection" ? "create" : destination;
    setMode(nextMode);
    if (destination === "collection" || destination === "create") {
      update("itemMode", destination === "collection" ? "collection" : "single");
      setActiveStep(0);
    }
    history.pushState(null, "", destinationHref(destination));
    setChainOpen(false);
    setNotice("");
    window.scrollTo(0, 0);
  }
      ...
      <div className="creation-grid">
        <section className="form-panel">
          <div className="mode-toggle-card">
            <div className="segmented"><button className={draft.itemMode === "single" ? "active" : ""} onClick={() => switchMode("create")}>Single item</button><button className={draft.itemMode === "collection" ? "active" : ""} onClick={() => switchMode("collection")}>Collection (multiple items)</button></div>
            {draft.itemMode === "collection" && <p className="field-hint">Import traits from CSV or metadata files, or edit each item below. Generative layering is not included.</p>}
          </div>
          {draft.itemMode === "single" ? (
            <>
              ...
              >
                ...
                {artwork ? <><span className="file-kicker">{artwork.kind.toUpperCase()} · {artwork.extension}</span><h2>{artwork.name}</h2><p>{artwork.size} · Click to replace</p></> : <><span className="file-kicker">UNIVERSAL ASSET INPUT</span><h2>Drop literally any file</h2><p>Images, video, audio, 3D, PDFs, HTML, code, archives and more · Up to 90 MB</p></>}
            ...
            <>
              <div className={dragging ? "upload-zone dragging" : "upload-zone"} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void acceptCollectionFiles(event.dataTransfer.files); }} onClick={() => collectionFileRef.current?.click()} role="button" aria-label="Upload collection files" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); collectionFileRef.current?.click(); } }}>
                <input ref={collectionFileRef} type="file" multiple onClick={(event) => event.stopPropagation()} onChange={(e) => { void acceptCollectionFiles(e.target.files); e.target.value = ""; }} aria-label="Choose collection files" hidden />
                <div className="upload-symbol"><span>↑</span></div>
              ...
              <p className="field-hint">Drag and drop or select up to 10,000 artwork files plus 10,000 matching JSON files · 90 MiB per file. Add traits below, then continue to Collection to set the name, banner and links.</p>
              <div className="csv-import-row" onClick={(event) => event.stopPropagation()}>
                  ...
                  <button type="button" className="secondary-action" disabled={!collectionItems.length} onClick={() => csvFileRef.current?.click()}>Import trait CSV</button>
                  <button type="button" className="secondary-action" disabled={!collectionItems.length} onClick={() => jsonFileRef.current?.click()}>Import metadata JSON</button>
                      ...
                      {pagedItems.map((item, localIndex) => (
                        <div className="collection-item-card" key={item.id}>
                          <div className="collection-item-head">
                            <CollectionThumbnail file={item.file} />
        ...
        <aside className="preview-column">
          ...
          <div className="storage-note"><span>◎</span><div><b>Any file. Content-addressed.</b><p>Files and metadata upload to Irys in resumable batches. Review the storage quote before paying.</p></div></div>
        </aside>
      </div>
    );
  }
    ...
    return (
      <div className="settings-layout">
        <section className="settings-main">
          <div className="settings-card"><div className="card-heading"><div><span>COLLECTION OWNERSHIP</span><h2>A collection you own</h2></div></div><p>Your wallet owns an isolated collection contract created through MintForge’s shared deployment infrastructure. Manage existing MintForge collections in the dashboard.</p></div>
          ...
          <div className="settings-card">
            ...
            <div className="form-fields compact">
              ...
              <label className="field">
                <span>SYMBOL</span>
                <input value={draft.symbol} onChange={(e) => update("symbol", e.target.value.toUpperCase().slice(0, 8))} placeholder="FORGE" /><small>Symbols do not need to be unique; the contract address identifies your collection.</small>
              </label>
          ...
          <div className="settings-card">
            ...
            <div className="choice-grid two tight">
              ...
              <ChoiceCard selected={draft.standard === "ERC-404"} onClick={() => update("standard", "ERC-404")} tag="EXPERIMENTAL" title="ERC-404" copy="Linked fungible tokens and NFTs. Fractional transfers can bank or restore NFT IDs." />
            </div>
          </div>
        </section>
        <aside className="summary-rail">
          ...
          <div className="contract-card">
            ...
            <span>CREATOR-OWNED COLLECTION</span>
            <h3>{draft.collectionName || "Untitled Collection"}</h3>
            <dl><div><dt>Network</dt><dd>{selectedChain.name}</dd></div><div><dt>Standard</dt><dd>{draft.standard}</dd></div><div><dt>Owner</dt><dd>{wallet ? shortWallet : "Connect wallet"}</dd></div><div><dt>Symbol</dt><dd>{draft.symbol || "—"}</dd></div></dl>

          </div>
          <div className="info-block"><b>Your collection, your wallet</b><p>MintForge cannot withdraw your creator proceeds. Your wallet approves collection management actions.</p></div>
        </aside>
      </div>
    );
  }
        ...
        <section className="settings-main">
          ...
          <div className="settings-card">
            ...
            <div className="toggle-list top-gap">
              <div><div><b>Allowlist phase</b><p>Restrict minting to approved wallets until you open the public mint from the dashboard.</p></div><Toggle checked={draft.allowlist} onChange={(v) => update("allowlist", v)} label="Allowlist phase" /></div>
              {draft.allowlist && (() => {
              ...
              <div><div><b>Delayed reveal</b><p>Show placeholder art until you reveal the collection. This is a display reveal; published metadata is not encrypted.</p></div><Toggle checked={draft.delayedReveal} onChange={(v) => update("delayedReveal", v)} label="Delayed reveal" /></div>
              {draft.delayedReveal && (
                <div className="sub-panel">
    ...
    return (
      <div className="settings-layout">
        <section className="settings-main">
          ...
          <div className="settings-card">
            ...
            <div className="fee-table">
              <div><span>Creator proceeds</span><b>{(draft.primarySplitEnabled ? (100 - (platformFee ?? 0)) * (creatorPercent / 100) : (100 - (platformFee ?? 0))).toFixed(2)}%</b></div>
              {draft.primarySplitEnabled && draft.collaborators.map((c, i) => (
                <div key={c.id}><span>Collaborator {i + 1} proceeds</span><b>{((100 - (platformFee ?? 0)) * c.percent / 100).toFixed(2)}%</b></div>
              ))}
              <div><span>Mint Forge platform fee</span><b>{platformFee === null ? "Not configured" : platformFee + "%"}</b></div>
        ...
        <aside className="summary-rail sticky-rail">
          ...
          <div className="info-block warning"><b>Marketplace note</b><p>Royalties are signalled on-chain, but enforcement depends on the marketplace. MintForge pays declared ERC-2981 royalties on completed marketplace sales.</p></div>
        </aside>
      </div>
    );
  }
      ...
      <div className="utility-stack">
          ...
          {draft.stakingEnabled && <div className="utility-body">
            <div className="staking-config-section">
              ...
              <div className="choice-grid two tight vault-options">
                <ChoiceCard selected={draft.stakeVaultMode === "shared"} onClick={() => update("stakeVaultMode", "shared")} tag="RECOMMENDED" title="Shared MintForge vault" copy="Use the MintForge staking vault deployed on this network."><span className="mini-detail">Isolated collection accounting · Lower deployment cost</span></ChoiceCard>
        ...
        <section className="utility-card enabled">
          ...
          <div className="utility-body toggle-list grid">
            ...
            <div><div><b>Emergency pause</b><p>Allow the owner to pause new mints. Holder transfers and staking exits remain available.</p></div><Toggle checked={draft.pausable} onChange={(v) => update("pausable", v)} label="Emergency pause" /></div>
        ...
        <section className="review-main">
          {draft.itemMode === "single" ? (
            <div className="review-hero">
              ...
              <div><span>READY TO DEPLOY</span><h2>{draft.name || "Untitled NFT"}</h2><p>{draft.collectionName} · {selectedChain.name}</p><div className="review-badges"><b>{draft.standard}</b><b>Creator-owned</b><b>{draft.mintType === "unique" ? "1 / 1" : draft.mintType + " edition"}</b></div></div>
            </div>
          ) : (
            <div className="review-hero">
              ...
              <div><span>READY TO DEPLOY</span><h2>{draft.collectionName || "Untitled Collection"}</h2><p>{collectionItems.length} item{collectionItems.length === 1 ? "" : "s"} · {selectedChain.name}</p><div className="review-badges"><b>{draft.standard}</b><b>Creator-owned</b><b>Collection</b></div></div>
            </div>
          )}
          <div className="review-grid">
            ...
            <div className="review-card"><span>REVENUE</span><dl><div><dt>Royalty</dt><dd>{draft.royaltyEnabled ? draft.royaltyPercent + "%" : "Off"}</dd></div><div><dt>Primary split</dt><dd>{draft.primarySplitEnabled ? draft.collaborators.length + " collaborator" + (draft.collaborators.length === 1 ? "" : "s") : "Creator only"}</dd></div><div><dt>Platform fee</dt><dd>{platformFee === null ? "Not configured" : platformFee + "%"}</dd></div><div><dt>Standard</dt><dd>ERC-2981</dd></div></dl><button onClick={() => goToStep(3)}>Edit revenue</button></div>
            ...
            <div className="review-card"><span>STORAGE</span><dl>{draft.itemMode === "single" ? (<><div><dt>Original payload</dt><dd>{artwork ? artwork.extension : "Needed"}</dd></div><div><dt>File category</dt><dd>{artwork ? artwork.kind : "—"}</dd></div></>) : (<><div><dt>Items</dt><dd>{collectionItems.length || "Needed"}</dd></div><div><dt>With traits</dt><dd>{collectionItems.filter((item) => item.traits.length > 0).length} of {collectionItems.length}</dd></div></>)}<div><dt>Metadata</dt><dd>Irys</dd></div><div><dt>Network</dt><dd>{selectedChain.name}</dd></div></dl><button onClick={() => goToStep(0)}>Edit asset</button></div>
          </div>
        </section>
        <LaunchCollection draft={draft} artwork={artwork} items={collectionItems} placeholder={revealPlaceholder} banner={banner} chain={selectedChain} wallet={wallet} onConnect={connectWallet} />
      </div>
    );
  }
  ...
  return (
    <main className={"app-shell mode-" + mode}>
      <aside className="sidebar">
        ...
        <div className="phase-note"><span>{mode === "create" ? "PROTOCOL STATUS" : mode === "viewer" ? "READ-ONLY VIEWER" : mode === "dashboard" ? "DASHBOARD STATUS" : mode === "market" ? "MARKETPLACE STATUS" : "STAKING STATUS"}</span><p>{mode === "create" ? "Create a collection owned by your wallet." : mode === "viewer" ? "No wallet or blockchain approval required." : mode === "dashboard" ? "Saved drafts, mint progress and collection controls." : mode === "market" ? "Browse NFTs, manage listings and make offers." : "Funded rewards · on-chain NFT custody."}</p></div>
        <a className="help-button" href="/guide">Documentation <span>↗</span></a>
      </aside>
      ...
      <section className="workspace" id="top">
        <header className="topbar">
          ...
          <StudioNavigation active={mode === "create" && draft.itemMode === "collection" ? "collection" : mode} onNavigate={switchMode} />
        ...
        <div className="page-wrap">
          {notice && <div className="notice" role="status"><span>i</span>{notice}<button type="button" onClick={() => setNotice("")} aria-label="Dismiss notice">×</button></div>}
          {mode === "viewer" ? <NftViewer chain={selectedChain} wallet={wallet} onConnectWallet={connectWallet} /> : mode === "stake" ? <NftStaking chain={selectedChain} wallet={wallet} onConnectWallet={connectWallet} /> : mode === "dashboard" ? <CreatorDashboard chain={selectedChain} wallet={wallet} onConnectWallet={connectWallet} onEditDraft={() => switchMode(draft.itemMode === "collection" ? "collection" : "create")} onCreateCollection={() => switchMode("collection")} onManageStaking={() => switchMode("stake")} /> : mode === "market" ? <Marketplace chain={selectedChain} wallet={wallet} onConnectWallet={connectWallet} /> : <>
            ...
            <footer className="page-actions">
              ...
              <span>{activeStep === 5 ? "Review the deployment before approving it in your wallet." : "Your progress is stored on this device."}</span>
