...
const NETWORKS: Record<string, { name: string; rpc: string }> = {
  ...
  "0xaa36a7": { name: "Sepolia testnet", rpc: "https://ethereum-sepolia-rpc.publicnode.com" },
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    let tokenUri: string;
    let standard: "ERC-721" | "ERC-1155" | "ERC-404";
    try {
      tokenUri = decodeAbiString(await contractCall(network.rpc, contract, TOKEN_URI_SELECTOR + tokenHex));
      standard = "ERC-721";
      try {
        if (BigInt(await contractCall(network.rpc, contract, "0x6e8f624b")) === 1n << 255n) standard = "ERC-404";
      } catch { /* ordinary ERC-721 */ }
    } catch {
      try {
