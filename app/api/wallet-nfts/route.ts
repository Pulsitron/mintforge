...
async function fetchIndexedTotal(base: string, wallet: string) {
  // Count only when the indexer can supply a complete result in one response.
  // Fetching every collection page delayed the first gallery batch; partial
  // counts must never be presented as the wallet's exact total.
  const body = await fetchIndexerPage(base, wallet, {}, true);
  if (Object.keys(paginationQuery(body.next_page_params)).length) return null;
  let total = 0;
  for (const item of Array.isArray(body.items) ? body.items : []) {
    const count = Number(recordFrom(item).amount ?? 0);
